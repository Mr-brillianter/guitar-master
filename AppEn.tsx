      ...
      if (!isMovable && shape.name === 'C') {
         ...
         notes = OPEN_C_FINGERING.map(pos => ({
          ...
          notes = notes.map(n => {
              ...
              if (n.finger === 0) {
                  newFinger = 1; // Open string -> Index Barre
              } else {
                  newFinger = Math.min(4, n.finger + 1); // Shift fingers up
              }
  ...
  return (
    <div className="min-h-screen p-6 md:p-12 flex flex-col items-center">
      ...
      <header className="mb-10 text-center space-y-2">
        ...
        <p className="text-slate-400 font-medium">Full Chromatic Scale Chord Workout</p>
      </header>
      ...
      <main className="w-full max-w-4xl grid gap-8">
        ...
        <div className="bg-slate-800/50 backdrop-blur-sm p-6 rounded-2xl border border-slate-700/50 flex flex-col md:flex-row items-center justify-between gap-6 shadow-xl">
          ...
          <div className="flex flex-col gap-2">
              <label className="text-xs font-bold text-slate-500 uppercase tracking-wider">Select Key</label>
        ...
        {currentChord && (
             <div className="flex flex-col items-center animate-fadeIn">
                 <div className="flex items-baseline gap-3 mb-6">
                     ...
                     <span className="text-2xl font-light text-slate-400">Major</span>
                     <span className="px-4 py-1 rounded-full bg-slate-700 text-cyan-300 font-mono text-lg border border-slate-600">
                         {currentChord.shapeName}-Shape
                     </span>
                     <span className="text-slate-500 font-mono">
                         Pos: {currentChord.fretPosition === 0 ? 'Open' : currentChord.fretPosition}
                     </span>
                 </div>

                 <Fretboard chord={currentChord} lang="en" />
             </div>
        )}
        ...
        <div className="mt-8 bg-slate-800/30 border border-slate-700/50 p-4 rounded-lg flex items-start gap-3 max-w-2xl mx-auto">
             ...
             <div className="space-y-2 text-sm text-slate-300">
                 <p>
                     <span className="font-bold text-slate-200">Pro Tip: </span> 
                     For Movable shapes (Barre chords), the 
                     <span className="text-amber-400 font-bold mx-1">1</span> (Index) becomes the nut/barre. 
                     Other fingers shift up: Index becomes <span className="text-amber-400 font-bold">2</span> (Middle), 
                     Middle becomes <span className="text-amber-400 font-bold">3</span> (Ring), 
                     and Ring becomes <span className="text-amber-400 font-bold">4</span> (Pinky).
                 </p>
             </div>
        </div>
